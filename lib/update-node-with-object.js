    ...
    switch (key) {
      case 'selectors':
        ...
        for (var selector in selectors) {
          ...
          $node.querySelector(selector).innerHTML = selectors[selector]
      ...
      case 'innerText':
        ...
        if (obj[key] && obj[key].regex || typeof obj[key] === 'function') {
          //$node.each(function (i, node) {
            ...
            var newText = newValue($node.innerText, obj[key])
      ...
      default:
        if (obj[key] && obj[key].regex || typeof obj[key] === 'function') {
          //$node.each(function (i, node) {
            var newText = newValue($node.getAttribute(key), obj[key])
            $node.setAttribute(key, newText)
          //})
        } else {
